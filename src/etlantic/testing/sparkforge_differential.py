...
outcomes, classification) — never backend DataFrame objects.
"""
...
class SparkForgeDifferentialFixture:
    """One classified SparkForge ``pipeline_builder`` IR fixture."""
...
@dataclass(frozen=True, slots=True)
class SparkForgeDifferentialResult:
...
def run_sparkforge_differential_suite(
    fixtures: Iterable[SparkForgeDifferentialFixture] | None = None,
    *,
    adapt_pipeline: Any | None = None,
) -> list[SparkForgeDifferentialResult]:
    ...
    write modes when provided. ``plugin_dependent`` fixtures must adapt when
    Delta ops are soft-warned, and record required plugin capabilities.
    """
    ...
    results: list[SparkForgeDifferentialResult] = []
    for fixture in corpus:
        ...
        strict_delta = fixture.classification != "plugin_dependent"
        try:
            adapted = adapt_pipeline(spec, strict_delta=strict_delta)
        except Exception as exc:
            results.append(
                SparkForgeDifferentialResult(
                    fixture_id=fixture.fixture_id,
...
def default_sparkforge_fixtures() -> list[SparkForgeDifferentialFixture]:
    ...
    candidates = [
        ...
        / "fixtures"
        / "sparkforge",
        Path.cwd() / "tests" / "medallantic" / "fixtures" / "sparkforge",
    ]
    root = next((p for p in candidates if p.is_dir()), None)
    if root is None:
        return []
    fixtures: list[SparkForgeDifferentialFixture] = []
