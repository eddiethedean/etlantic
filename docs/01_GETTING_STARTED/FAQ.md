...
## Is ETLantic 0.31 production-supported?

ETLantic **0.31.0** is a **Beta** (PyPI) release suitable for documented single-tenant reference
...
**Available** APIs and behaviors are supported within the documented 0.31
...
reproducible 0.31.0 environment, pin both exactly, for example:

```bash
python -m pip install 'etlantic==0.31.0' 'etlantic-polars==0.31.0'
```
