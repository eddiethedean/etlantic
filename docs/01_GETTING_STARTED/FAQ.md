...
## Is ETLantic 0.30 production-supported?

ETLantic **0.30.0** is a **Beta** (PyPI) release suitable for documented single-tenant reference
...
**Available** APIs and behaviors are supported within the documented 0.30
...
reproducible 0.30.0 environment, pin both exactly, for example:

```bash
python -m pip install 'etlantic==0.30.0' 'etlantic-polars==0.30.0'
```
