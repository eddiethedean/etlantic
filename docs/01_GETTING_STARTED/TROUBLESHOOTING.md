...
Those packages ship with ETLantic 0.53.0 as separate distributions. Keep every
...
Core **0.53.x** requires official plugins from the **same** minor
...
`etlantic-polars==0.53.0` into the same environment as core. Entry-point
...
`"etlantic-polars": "==0.53.0"`. The allowlist permits discovery; it does not
...
|---|---|---|
| Polars on an `init` project | `pip install 'etlantic[polars]==0.53.0'` | [Polars tutorial](../06_EXECUTION/POLARS_TUTORIAL.md) |
| Pandas on an `init` project | `pip install 'etlantic[pandas]==0.53.0'` | [Pandas tutorial](../06_EXECUTION/PANDAS_TUTORIAL.md) |
| SQL hello (SQLite) | `pip install 'etlantic[sql]==0.53.0'` | [SQL hello (PyPI)](../06_EXECUTION/SQL_HELLO_PYPI.md) |
...
`etlantic-pyspark==0.53.0`. Check both from the same environment:
...
Install `etlantic-sql==0.53.0`, select `Profile(sql_engine="sql")`, and provide
...
0.53.0 wheel in your environment. Check the imported path:
