...
These docs describe ETLantic **0.23.0**. Confirm what you installed:
...
Those packages ship with ETLantic 0.23.0 as separate distributions. Keep every
...
python -m pip install --upgrade \
  'etlantic-polars==0.23.0' 'etlantic-pandas==0.23.0'
python -m pip install --upgrade \
  'etlantic-sql==0.23.0' 'etlantic-pyspark==0.23.0'
python -m pip install --upgrade \
  'etlantic-airflow==0.23.0' 'etlantic-prefect==0.23.0'
python -m pip install --upgrade \
  'etlantic-sparkforge==0.23.0'
```
...
python -m pip install --upgrade --force-reinstall \
  'etlantic==0.23.0' 'etlantic-polars==0.23.0'
```
...
If the map is empty, install a matching `etlantic-polars==0.23.0` into the same
...
`"etlantic-polars": "==0.23.0"`. The allowlist permits discovery; it does not
...
|---|---|---|
| Polars portable kernel | `pip install 'etlantic-polars==0.23.0'` or `uv sync --group dataframes` | checkout `examples/portable_polars_kernel.py` |
| Polars / Pandas native | `pip install 'etlantic-polars==0.23.0' 'etlantic-pandas==0.23.0'` or `uv sync --group dataframes` | checkout `examples/dataframe_parity.py` |
| Polars ↔ Pandas Gate A | same as above | checkout `examples/interchange_polars_pandas.py` |
| SQL | `pip install 'etlantic-sql==0.23.0'` or `uv sync --group sql` | checkout `examples/sql_to_sql.py` |
| PySpark | `pip install 'etlantic-pyspark==0.23.0'` or `uv sync --group pyspark` | checkout `examples/pyspark_local.py` |
| Airflow compile | `pip install 'etlantic-airflow==0.23.0'` or `uv sync --group airflow` | checkout `examples/airflow_compile.py` |
| SparkForge adapter | `pip install 'etlantic-sparkforge==0.23.0'` or `uv sync --group sparkforge` | `tests/sparkforge/` |
...
| Symptom | Fix |
|---|---|
| Plugin not discovered | Install `etlantic-polars==0.23.0` **and** `etlantic-pandas==0.23.0`; match core minor |
...
`etlantic-pyspark==0.23.0`. Check both from the same environment:
...
Install `etlantic-sql==0.23.0`, select `Profile(sql_engine="sql")`, and provide
