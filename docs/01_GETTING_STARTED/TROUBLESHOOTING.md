...
These docs describe ETLantic **0.31.0**. Confirm what you installed:
...
Upgrade from PyPI (pin the published **0.31.0** release for reproducible installs):

```bash
python -m pip install --upgrade 'etlantic==0.31.0'
# or accept compatible 0.31.x patches within the minor:
python -m pip install --upgrade 'etlantic>=0.31.0,<0.32'
```
...
Those packages ship with ETLantic 0.31.0 as separate distributions. Keep every
...
python -m pip install --upgrade \
  'etlantic-polars==0.31.0' 'etlantic-pandas==0.31.0'
python -m pip install --upgrade \
  'etlantic-sql==0.31.0' 'etlantic-pyspark==0.31.0'
python -m pip install --upgrade \
  'etlantic-airflow==0.31.0' 'etlantic-prefect==0.31.0'
python -m pip install --upgrade \
  'medallantic==0.31.0'
```
...
Core **0.31.x** requires official plugins from the **same** minor
(`0.31.x`). Do not mix plugins from a different minor with core (for example
`0.25` plugins with `0.26` core). Remove stale plugin
versions and install matching pins, for example:

```bash
python -m pip install --upgrade --force-reinstall \
  'etlantic==0.31.0' 'etlantic-polars==0.31.0'
```
...
If the map is empty, install a matching `etlantic-polars==0.31.0` into the same
...
| `PMPLUG403` | Allowlist pin is not a valid version specifier | Fix the pin syntax (`==0.31.0`, `>=0.31.0,<0.32`, …) |
...
`"etlantic-polars": "==0.31.0"`. The allowlist permits discovery; it does not
...
|---|---|---|
| Polars portable kernel | `pip install 'etlantic-polars==0.31.0'` or `uv sync --group dataframes` | checkout `examples/portable_polars_kernel.py` |
| Polars / Pandas native | `pip install 'etlantic-polars==0.31.0' 'etlantic-pandas==0.31.0'` or `uv sync --group dataframes` | checkout `examples/dataframe_parity.py` |
| Polars ↔ Pandas Gate A | same as above | checkout `examples/interchange_polars_pandas.py` |
| SQL | `pip install 'etlantic-sql==0.31.0'` or `uv sync --group sql` | checkout `examples/sql_to_sql.py` |
| PySpark | `pip install 'etlantic-pyspark==0.31.0'` or `uv sync --group pyspark` | checkout `examples/pyspark_local.py` |
| Airflow compile | `pip install 'etlantic-airflow==0.31.0'` or `uv sync --group airflow` | checkout `examples/airflow_compile.py` |
| Medallantic | `pip install 'medallantic==0.31.0'` or `uv sync --group medallantic` | `tests/medallantic/` |
...
Pandas boundaries and remains available in 0.31.

| Symptom | Fix |
|---|---|
| Plugin not discovered | Install `etlantic-polars==0.31.0` **and** `etlantic-pandas==0.31.0`; match core minor |
...
`etlantic-pyspark==0.31.0`. Check both from the same environment:
...
Install `etlantic-sql==0.31.0`, select `Profile(sql_engine="sql")`, and provide
...
0.31.0 wheel in your environment. Check the imported path:
