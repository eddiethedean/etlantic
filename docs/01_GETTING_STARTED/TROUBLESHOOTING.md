...
These docs describe ETLantic **0.33.0**. Confirm what you installed:
...
Upgrade from PyPI (pin the published **0.33.0** release for reproducible installs):

```bash
python -m pip install --upgrade 'etlantic==0.33.0'
# or accept compatible 0.33.x patches within the minor:
python -m pip install --upgrade 'etlantic>=0.33.0,<0.34'
```
...
Those packages ship with ETLantic 0.33.0 as separate distributions. Keep every
...
python -m pip install --upgrade \
  'etlantic-polars==0.33.0' 'etlantic-pandas==0.33.0'
python -m pip install --upgrade \
  'etlantic-sql==0.33.0' 'etlantic-pyspark==0.33.0'
python -m pip install --upgrade \
  'etlantic-airflow==0.33.0' 'etlantic-prefect==0.33.0'
python -m pip install --upgrade \
  'medallantic==0.33.0'
```
...
Core **0.33.x** requires official plugins from the **same** minor
(`0.33.x`). Do not mix plugins from a different minor with core (for example
...
`etlantic-polars==0.33.0` into the same environment as core. Entry-point
...
| `PMPLUG403` | Allowlist pin is not a valid version specifier | Fix the pin syntax (`==0.33.0`, `>=0.33.0,<0.34`, …) |
...
`"etlantic-polars": "==0.33.0"`. The allowlist permits discovery; it does not
...
| Symptom | Fix |
|---|---|
| Plugin not discovered | Install `etlantic-polars==0.33.0` **and** `etlantic-pandas==0.33.0`; match core minor |
...
`etlantic-pyspark==0.33.0`. Check both from the same environment:
...
Install `etlantic-sql==0.33.0`, select `Profile(sql_engine="sql")`, and provide
...
0.33.0 wheel in your environment. Check the imported path:
