...
These docs describe ETLantic **0.25.1**. Confirm what you installed:
...
Upgrade from PyPI (pin the published **0.25.0** release for reproducible installs):

```bash
python -m pip install --upgrade 'etlantic==0.25.1'
# or accept compatible 0.25.x patches within the minor:
python -m pip install --upgrade 'etlantic>=0.25.0,<0.26'
```
...
Those packages ship with ETLantic 0.25.1 as separate distributions. Keep every
...
python -m pip install --upgrade \
  'etlantic-polars==0.25.1' 'etlantic-pandas==0.25.1'
python -m pip install --upgrade \
  'etlantic-sql==0.25.1' 'etlantic-pyspark==0.25.1'
python -m pip install --upgrade \
  'etlantic-airflow==0.25.1' 'etlantic-prefect==0.25.1'
python -m pip install --upgrade \
  'etlantic-sparkforge==0.25.1'
```
...
python -m pip install --upgrade --force-reinstall \
  'etlantic==0.25.1' 'etlantic-polars==0.25.1'
```
...
If the map is empty, install a matching `etlantic-polars==0.25.1` into the same
...
| `PMPLUG403` | Allowlist pin is not a valid version specifier | Fix the pin syntax (`==0.25.1`, `>=0.25.0,<0.26`, …) |
...
`"etlantic-polars": "==0.25.1"`. The allowlist permits discovery; it does not
...
|---|---|---|
| Polars portable kernel | `pip install 'etlantic-polars==0.25.1'` or `uv sync --group dataframes` | checkout `examples/portable_polars_kernel.py` |
| Polars / Pandas native | `pip install 'etlantic-polars==0.25.1' 'etlantic-pandas==0.25.1'` or `uv sync --group dataframes` | checkout `examples/dataframe_parity.py` |
| Polars ↔ Pandas Gate A | same as above | checkout `examples/interchange_polars_pandas.py` |
| SQL | `pip install 'etlantic-sql==0.25.1'` or `uv sync --group sql` | checkout `examples/sql_to_sql.py` |
| PySpark | `pip install 'etlantic-pyspark==0.25.1'` or `uv sync --group pyspark` | checkout `examples/pyspark_local.py` |
| Airflow compile | `pip install 'etlantic-airflow==0.25.1'` or `uv sync --group airflow` | checkout `examples/airflow_compile.py` |
| SparkForge adapter | `pip install 'etlantic-sparkforge==0.25.1'` or `uv sync --group sparkforge` | `tests/sparkforge/` |
...
| Symptom | Fix |
|---|---|
| Plugin not discovered | Install `etlantic-polars==0.25.1` **and** `etlantic-pandas==0.25.1`; match core minor |
...
`etlantic-pyspark==0.25.1`. Check both from the same environment:
...
Install `etlantic-sql==0.25.1`, select `Profile(sql_engine="sql")`, and provide
...
0.25.0 wheel in your environment. Check the imported path:
