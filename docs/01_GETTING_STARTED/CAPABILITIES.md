...
| Adaptive heterogeneous planning and executable physical DAGs | **Planned, not currently available** for 0.49: opt-in static-batch local execution with bounded `/2` plans; proposed qualification is Local/Polars/Pandas single-target plus directional Polars↔Pandas Gate A — see [implementation plan](../11_DEVELOPMENT/IMPLEMENTATION_PLAN_0_49.md), [not-started exit gate](../11_DEVELOPMENT/EXIT_GATE_0_49.md), and [epic #30](https://github.com/eddiethedean/etlantic/issues/30) |
