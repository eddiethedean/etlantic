...
or lossless `etlantic.pipeline/1` JSON. ETLantic **0.29.0** is a **Beta**
...
| `etlantic-datafusion` experimental engine | **Experimental in 0.29.0** (Gate B; not graduated) |
...
| Production FastAPI control plane | Continues in 1.1 (0.29 ships only the thin reference adapter) |
| Medallantic transform execution | Native authoring ships in 0.29; callable/rule execution is 0.30+ |
...
**Shipped in 0.28–0.29** (not residual): quadruple-minor burn-in proof and Plugin SDK
`/1` freeze for core families. See
[What's New in 0.29](WHATS_NEW_0_29.md) and
...
```bash
pip install 'etlantic==0.29.0'                 # core only — no engines
pip install 'etlantic-polars==0.29.0'          # Polars reference plugin
pip install 'etlantic-pandas==0.29.0'          # Pandas compatibility plugin
pip install 'etlantic-sql==0.29.0'             # PostgreSQL SQL reference plugin
pip install 'etlantic-pyspark==0.29.0'         # PySpark reference plugin
pip install 'etlantic-airflow==0.29.0'         # Airflow DAG compiler
pip install 'etlantic-prefect==0.29.0'         # Prefect direct-execution scheduler
pip install 'etlantic-keyring==0.29.0'         # OS keyring secret provider
pip install 'etlantic-sqlmodel==0.29.0'        # SQLModel contract bridge
pip install 'medallantic==0.29.0'              # medallion facade (native + IR migrate)
```
