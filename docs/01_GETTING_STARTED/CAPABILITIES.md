...
> **Status: Available in ETLantic 0.52.1 (published Beta).** What is implemented and technically
...
ETLantic 0.52.1 is the **published Beta release** for documented, controlled,
single-tenant pilots (install `etlantic==0.52.1` from PyPI). You can embed an
...
(after installation → `python -m etlantic init` → validate →
run). Do not start from repository `examples/` unless you have cloned the repo.
Headline tutorials: [Human-governed AI](HUMAN_GOVERNED_AI.md) and
...
| Engines | Portable baseline technically qualified on Local, Polars, Pandas, SQL, PySpark, DataFusion, and DuckDB; advanced profiles remain engine-specific |
...
## Supported standards policy (0.50)

For the 0.50 envelope, ETLantic supports these standards and
toolkits at the declared ranges (exact pins and protocol notes:
[Compatibility](../10_REFERENCE/COMPATIBILITY.md)):

| Standard / surface | 0.50 policy |
|---|---|
...
| Managed Spark providers (Databricks/EMR/Connect) | Kubernetes Job + Spark Connect **Experimental fakes** ship in 0.47; live provider packs remain planned for 0.57 |
...
| Adaptive Profile, physical-DAG wire format, placement planning, lowering, and explain/diff | **Available in 0.52 for deterministic, planning-only `/2` generation and inspection; execution remains unavailable** — see the [0.52 plan](../11_DEVELOPMENT/IMPLEMENTATION_PLAN_0_52.md), [exit gate](../11_DEVELOPMENT/EXIT_GATE_0_52.md), and [generated evidence](../11_DEVELOPMENT/evidence/adaptive_0_51/FINDINGS.md) |
...
| Dagster / expanded Prefect / Argo compilers | Planned brownfield bridges in 0.55 |
| Read-only-first operator console | Planned first-class for 0.56 |
| AWS/Azure/GCP/Vault secret-provider packs | Planned as optional providers in 0.57 |
| TransformationModel incubation | Deferred to 0.58 |
...
| Dedicated multi-worker / multi-tenant ops control plane | Partial — Supported profiles via CP-GA; Operator Console remains 0.56 ([plan](../11_DEVELOPMENT/MULTI_TENANT_CONTROL_PLANE_PLAN.md)) |
...
(the sample uses Polars — install `etlantic-polars==0.52.1` first).
...
```bash
pip install 'etlantic==0.52.1'
pip install 'etlantic[lsp]==0.52.1'            # optional language server
pip install 'etlantic-polars==0.52.1'          # optional
pip install 'etlantic-pandas==0.52.1'          # optional
pip install 'etlantic-sql==0.52.1'             # optional
pip install 'etlantic-pyspark==0.52.1'         # optional
pip install 'etlantic-airflow==0.52.1'         # optional
pip install 'etlantic-prefect==0.52.1'         # optional
pip install 'etlantic-keyring==0.52.1'         # optional
pip install 'etlantic-sqlmodel==0.52.1'        # optional
pip install 'medallantic==0.52.1'              # optional
```
