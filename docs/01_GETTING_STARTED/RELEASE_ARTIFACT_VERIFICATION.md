...
## What shipped for v0.36.0

| Artifact | Status |
|---|---|
| PyPI wheels/sdists for core and official packages | Published (`etlantic==0.36.0`, matching plugins) |
| Per-artifact SHA-256 manifest | [release-artifacts.json](https://github.com/eddiethedean/etlantic/releases/download/v0.36.0/release-artifacts.json) |
| GitHub build provenance attestations | Attached to release workflow / verifiable with `gh attestation verify` |
| CycloneDX environment SBOM | **Failed** — see [sbom-warning.txt](https://github.com/eddiethedean/etlantic/releases/download/v0.36.0/sbom-warning.txt) |
...
1. Download the wheel (or sdist) and `release-artifacts.json` from the
   [GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.36.0).
2. Confirm the file’s SHA-256 matches the manifest entry for that basename.
3. Prefer exact pins (`etlantic==0.36.0`) in lockfiles.

## Verify attestations

```bash
gh attestation verify path/to/etlantic-0.36.0-*.whl \
  --owner eddiethedean \
  --repo etlantic
```
