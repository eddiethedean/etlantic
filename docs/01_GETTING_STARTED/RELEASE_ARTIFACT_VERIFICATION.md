...
## Release artifact contract for v0.50.1

| Artifact | Status |
|---|---|
| PyPI wheels/sdists for core and official packages | Published (etlantic==0.50.1, matching plugins) |
| Per-artifact SHA-256 manifest | [release-artifacts.json](https://github.com/eddiethedean/etlantic/releases/download/v0.50.1/release-artifacts.json) |
...
1. Download the wheel (or sdist) and `release-artifacts.json` from the
   [GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.50.1).
2. Confirm the file’s SHA-256 matches the manifest entry for that basename.
3. Prefer exact pins (`etlantic==0.50.1`) in lockfiles.
