...
## Release artifact contract for v0.38.0

| Artifact | Status |
|---|---|
| PyPI wheels/sdists for core and official packages | Expected after the tag workflow succeeds (`etlantic==0.38.0`, matching plugins) |
| Per-artifact SHA-256 manifest | Expected as [release-artifacts.json](https://github.com/eddiethedean/etlantic/releases/download/v0.38.0/release-artifacts.json) |
...
1. After publication, download the wheel (or sdist) and
   ...
   [GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.38.0).
2. Confirm the file’s SHA-256 matches the manifest entry for that basename.
3. Prefer exact pins (`etlantic==0.38.0`) in lockfiles.
