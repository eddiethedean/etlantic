# Installing ETLantic 0.52.1

> **Status: Available in ETLantic 0.52.1 (published Beta).**

ETLantic **0.52.1** supports Python 3.11–3.13 and is published on
[PyPI](https://pypi.org/project/etlantic/0.52.1/).
...
the interpreter you intend is the one that runs. Pin **0.52.1** for
reproducible evaluation.

### pip

The pinned PyPI commands below install the published `0.52.1` release.

```bash
python -m venv .venv
source .venv/bin/activate   # Windows PowerShell: .\.venv\Scripts\Activate.ps1
python -m pip install --upgrade pip
python -m pip install 'etlantic==0.52.1'
...
uv pip install 'etlantic==0.52.1'
python -m etlantic --version
```

If you already have a uv project (`pyproject.toml`), you may use
`uv add 'etlantic==0.52.1'` instead. Create an **empty subdirectory** for
...
py -3.11 -m pip install 'etlantic==0.52.1'
...
explicitly and **match the core patch** (`0.52.1` with `0.52.1`).

**Primary install (separate packages):**

```bash
python -m pip install 'etlantic-polars==0.52.1'     # dataframe + Polars portable compiler
python -m pip install 'etlantic-pandas==0.52.1'     # dataframe + Pandas portable compiler
python -m pip install 'etlantic-sql==0.52.1'        # SQL plugin (SQLite + PostgreSQL Tier A)
python -m pip install 'etlantic-pyspark==0.52.1'    # PySpark plugin + portable compiler
python -m pip install 'etlantic-airflow==0.52.1'    # Airflow DAG compiler
python -m pip install 'etlantic-prefect==0.52.1'    # Prefect direct-execution (local MVP)
python -m pip install 'etlantic-keyring==0.52.1'    # OS keyring secret provider
python -m pip install 'etlantic-sqlmodel==0.52.1'   # SQLModel bridge helpers
python -m pip install 'medallantic==0.52.1' # SparkForge → ETLantic IR adapter
```

**Equivalent extras** (same packages, same pins):

```bash
python -m pip install 'etlantic[polars]==0.52.1'
python -m pip install 'etlantic[pandas]==0.52.1'
python -m pip install 'etlantic[dataframes]==0.52.1'   # polars + pandas
python -m pip install 'etlantic[sql]==0.52.1'          # alias: [postgresql]
python -m pip install 'etlantic[pyspark]==0.52.1'      # alias: [spark]
python -m pip install 'etlantic[airflow]==0.52.1'
python -m pip install 'etlantic[prefect]==0.52.1'
# Experimental Gate B stub (not graduated; not recommended):
python -m pip install 'etlantic[datafusion]==0.52.1'
python -m pip install 'etlantic[kafka]==0.52.1'        # Experimental FakeKafka
python -m pip install 'etlantic[schemaregistry]==0.52.1'  # Experimental fake registry
```
...
```bash
python -m pip install 'etlantic-fastapi==0.52.1'   # dual surface: CP1 ETLanticAPI + thin create_reference_app (non-CP); CP1 ≠ multi-tenant GA
# or: python -m pip install 'etlantic[fastapi]==0.52.1'
```
...
```bash
python -m pip install --upgrade 'etlantic==0.52.1'
```
...
source .venv/bin/activate
python -m pip install --upgrade pip
python -m pip install 'git+https://github.com/eddiethedean/etlantic.git@v0.52.1'
python -m etlantic --version
```
