...
Pin the published **0.25.0** release for reproducible evaluation. Use a virtual
...
explicitly and **match the core minor** (`0.25.0` with `0.25.0`).
...
source .venv/bin/activate
python -m pip install --upgrade pip
python -m pip install 'git+https://github.com/eddiethedean/etlantic.git@v0.25.0'
python -m etlantic --version
```
