...
Pin the published **0.28.0** release for reproducible evaluation. Use a virtual
...
uv pip install 'etlantic==0.28.0'
python -m etlantic --version
```
...
py -3.11 -m pip install 'etlantic==0.28.0'
...
```bash
python -m pip install 'etlantic-polars==0.28.0'     # dataframe + Polars portable compiler
python -m pip install 'etlantic-pandas==0.28.0'     # dataframe + Pandas portable compiler
python -m pip install 'etlantic-sql==0.28.0'        # PostgreSQL SQL reference plugin
python -m pip install 'etlantic-pyspark==0.28.0'    # PySpark plugin + portable compiler
python -m pip install 'etlantic-airflow==0.28.0'    # Airflow DAG compiler
python -m pip install 'etlantic-prefect==0.28.0'    # Prefect direct-execution (local MVP)
python -m pip install 'etlantic-keyring==0.28.0'    # OS keyring secret provider
python -m pip install 'etlantic-sqlmodel==0.28.0'   # SQLModel bridge helpers
python -m pip install 'medallantic==0.28.0' # SparkForge → ETLantic IR adapter
```
...
python -m pip install 'etlantic[pandas]==0.28.0'
python -m pip install 'etlantic[dataframes]==0.28.0'   # polars + pandas
python -m pip install 'etlantic[sql]==0.28.0'          # alias: [postgresql]
python -m pip install 'etlantic[pyspark]==0.28.0'      # alias: [spark]
python -m pip install 'etlantic[airflow]==0.28.0'
python -m pip install 'etlantic[prefect]==0.28.0'
# Experimental Gate B stub (not graduated; not recommended):
python -m pip install 'etlantic[datafusion]==0.28.0'
```
...
```bash
python -m pip install 'etlantic-fastapi==0.28.0'   # thin authoring/service HTTP reference (shipped since 0.24; not 1.1 control plane)
# or: python -m pip install 'etlantic[fastapi]==0.28.0'
```
...
source .venv/bin/activate
python -m pip install --upgrade pip
python -m pip install 'git+https://github.com/eddiethedean/etlantic.git@v0.28.0'
python -m etlantic --version
```
