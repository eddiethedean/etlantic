...
uv pip install 'etlantic==0.32.0'
python -m etlantic --version
```

If you already have a uv project (`pyproject.toml`), you may use
`uv add 'etlantic==0.32.0'` instead. Create an **empty subdirectory** for
...
```bash
python -m pip install 'etlantic-polars==0.32.0'     # dataframe + Polars portable compiler
python -m pip install 'etlantic-pandas==0.32.0'     # dataframe + Pandas portable compiler
python -m pip install 'etlantic-sql==0.32.0'        # PostgreSQL SQL reference plugin
python -m pip install 'etlantic-pyspark==0.32.0'    # PySpark plugin + portable compiler
python -m pip install 'etlantic-airflow==0.32.0'    # Airflow DAG compiler
python -m pip install 'etlantic-prefect==0.32.0'    # Prefect direct-execution (local MVP)
python -m pip install 'etlantic-keyring==0.32.0'    # OS keyring secret provider
python -m pip install 'etlantic-sqlmodel==0.32.0'   # SQLModel bridge helpers
python -m pip install 'medallantic==0.32.0' # SparkForge → ETLantic IR adapter
```
...
python -m pip install 'etlantic[pandas]==0.32.0'
python -m pip install 'etlantic[dataframes]==0.32.0'   # polars + pandas
python -m pip install 'etlantic[sql]==0.32.0'          # alias: [postgresql]
python -m pip install 'etlantic[pyspark]==0.32.0'      # alias: [spark]
python -m pip install 'etlantic[airflow]==0.32.0'
python -m pip install 'etlantic[prefect]==0.32.0'
# Experimental Gate B stub (not graduated; not recommended):
python -m pip install 'etlantic[datafusion]==0.32.0'
```
...
```bash
python -m pip install 'etlantic-fastapi==0.32.0'   # thin authoring/service HTTP reference (shipped since 0.24; not 1.1 control plane)
# or: python -m pip install 'etlantic[fastapi]==0.32.0'
```
...
source .venv/bin/activate
python -m pip install --upgrade pip
python -m pip install 'git+https://github.com/eddiethedean/etlantic.git@v0.32.0'
python -m etlantic --version
```
