# Installing ETLantic 0.42.0

> **Status: Available in ETLantic 0.42.0.**

ETLantic **0.42.0** supports Python 3.11–3.13 and is published on
[PyPI](https://pypi.org/project/etlantic/0.42.0/).
...
the interpreter you intend is the one that runs. Pin **0.42.0** for
...
uv pip install 'etlantic==0.42.0'
python -m etlantic --version
```

If you already have a uv project (`pyproject.toml`), you may use
`uv add 'etlantic==0.42.0'` instead. Create an **empty subdirectory** for
...
py -3.11 -m pip install 'etlantic==0.42.0'
...
explicitly and **match the core minor** (`0.42.0` with `0.42.0`).

**Primary install (separate packages):**

```bash
python -m pip install 'etlantic-polars==0.42.0'     # dataframe + Polars portable compiler
python -m pip install 'etlantic-pandas==0.42.0'     # dataframe + Pandas portable compiler
python -m pip install 'etlantic-sql==0.42.0'        # SQL plugin (SQLite + PostgreSQL Tier A)
python -m pip install 'etlantic-pyspark==0.42.0'    # PySpark plugin + portable compiler
python -m pip install 'etlantic-airflow==0.42.0'    # Airflow DAG compiler
python -m pip install 'etlantic-prefect==0.42.0'    # Prefect direct-execution (local MVP)
python -m pip install 'etlantic-keyring==0.42.0'    # OS keyring secret provider
python -m pip install 'etlantic-sqlmodel==0.42.0'   # SQLModel bridge helpers
python -m pip install 'medallantic==0.42.0' # SparkForge → ETLantic IR adapter
```

**Equivalent extras** (same packages, same pins):

```bash
python -m pip install 'etlantic[polars]==0.42.0'
python -m pip install 'etlantic[pandas]==0.42.0'
python -m pip install 'etlantic[dataframes]==0.42.0'   # polars + pandas
python -m pip install 'etlantic[sql]==0.42.0'          # alias: [postgresql]
python -m pip install 'etlantic[pyspark]==0.42.0'      # alias: [spark]
python -m pip install 'etlantic[airflow]==0.42.0'
python -m pip install 'etlantic[prefect]==0.42.0'
# Experimental Gate B stub (not graduated; not recommended):
python -m pip install 'etlantic[datafusion]==0.42.0'
```
...
```bash
python -m pip install 'etlantic-fastapi==0.42.0'   # dual surface: CP1 ETLanticAPI + thin create_reference_app (non-CP); CP1 ≠ multi-tenant GA
...
source .venv/bin/activate
python -m pip install --upgrade pip
python -m pip install 'git+https://github.com/eddiethedean/etlantic.git@v0.42.0'
python -m etlantic --version
```
