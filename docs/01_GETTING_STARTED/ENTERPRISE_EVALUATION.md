...
> **Status: Available in ETLantic 0.47.0.** Deep diligence packet. Start with the
...
1. [Installation](INSTALLATION.md) — `pip install etlantic==0.47.0`
...
| Artifact | Location |
|---|---|
| Version pins | Pin `etlantic==0.47.0` and matching plugin minors |
...
For a published GitHub Release asset (example: wheel from the `v0.47.0` release):
...
[v0.47.0 GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.47.0).
...
[Release artifact verification](RELEASE_ARTIFACT_VERIFICATION.md).
Prefer exact pins (`etlantic==0.47.0` and matching plugins) over floating
...
- HA/DR runbooks, Kubernetes reference architectures, or capacity SLAs in
  this Beta line; Kubernetes execution **fakes** are the 0.47 freeze (not
  started); live packs remain 0.51
...
- Practitioners: continue the [Current 0.47 Guide](CURRENT_VERSION.md)
