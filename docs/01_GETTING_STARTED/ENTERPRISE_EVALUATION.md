...
> **Status: Available in ETLantic 0.32.0.** Deep diligence packet. Start with the
...
For a published GitHub Release asset (example: wheel from the `v0.32.0` release):

```bash
# Download the wheel from the GitHub Release, then:
gh attestation verify path/to/etlantic-0.32.0-*.whl \
  --owner eddiethedean \
  --repo etlantic
```

SBOM digests are attached as release workflow artifacts / notes on the tagged
Release. Prefer exact pins (`etlantic==0.32.0` and matching plugins) over
...
- Practitioners: continue the [Current 0.32 Guide](CURRENT_VERSION.md)
