...
> **Status: Available in ETLantic 0.50.0 (published Beta).** Deep diligence packet. Start with the
...
1. [Installation](INSTALLATION.md) — `pip install etlantic==0.50.1`
...
| Artifact | Location |
|---|---|
| Version pins | Pin `etlantic==0.50.1` and matching plugin minors |
...
For a published GitHub Release asset (example: wheel from the `v0.50.1` release):

```bash
# Download the wheel from the GitHub Release, then:
gh attestation verify path/to/etlantic-0.50.0-*.whl \
  --owner eddiethedean \
  --repo etlantic
```
...
[v0.50.1 GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.50.1).
...
[Release artifact verification](RELEASE_ARTIFACT_VERIFICATION.md).
Prefer exact pins (`etlantic==0.50.1` and matching plugins) over floating
...
- Practitioners: continue the [Current 0.50 Guide](CURRENT_VERSION.md)
