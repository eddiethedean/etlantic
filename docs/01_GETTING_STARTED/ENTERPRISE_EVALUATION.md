...
> **Status: Available in ETLantic 0.44.0.** Deep diligence packet. Start with the
...
| Artifact | Location |
|---|---|
| Version pins | Pin `etlantic==0.44.0` and matching plugin minors |
...
For a published GitHub Release asset (example: wheel from the `v0.44.0` release):

```bash
# Download the wheel from the GitHub Release, then:
gh attestation verify path/to/etlantic-0.44.0-*.whl \
  --owner eddiethedean \
  --repo etlantic
```
...
[v0.44.0 GitHub Release](https://github.com/eddiethedean/etlantic/releases/tag/v0.44.0).
...
[Release artifact verification](RELEASE_ARTIFACT_VERIFICATION.md).
Prefer exact pins (`etlantic==0.44.0` and matching plugins) over floating
...
- Practitioners: continue the [Current 0.44 Guide](CURRENT_VERSION.md)
