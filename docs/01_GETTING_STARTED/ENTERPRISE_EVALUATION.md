...
> **Status: Available in ETLantic 0.40.0.** Deep diligence packet. Start with the
...
| Artifact | Location |
|---|---|
| Version pins | Pin `etlantic==0.40.0` and matching plugin minors |
...
For a published GitHub Release asset (example: wheel from the `v0.40.0` release):

```bash
# Download the wheel from the GitHub Release, then:
gh attestation verify path/to/etlantic-0.40.0-*.whl \
  --owner eddiethedean \
  --repo etlantic
```

After the v0.40.0 tag workflow succeeds, expect a per-artifact SHA-256
...
[Release artifact verification](RELEASE_ARTIFACT_VERIFICATION.md).
Prefer exact pins (`etlantic==0.40.0` and matching plugins) over floating
...
- Practitioners: continue the [Current 0.40 Guide](CURRENT_VERSION.md)
