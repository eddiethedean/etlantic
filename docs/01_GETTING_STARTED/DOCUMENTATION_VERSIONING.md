...
| Alias / slug | Meaning |
|---|---|
| **`/en/v0.50.1/`** | Immutable docs for the tagged release. Prefer this with `etlantic==0.50.1`. |
...
For `etlantic==0.50.1`, use the immutable `/en/v0.50.1/` documentation.
...
2. Activate the git tag `v0.50.1` (build if inactive).
3. Keep **latest** = `main` and **stable** = newest published tag.
4. Confirm `https://etlantic.readthedocs.io/en/v0.50.1/` returns 200.
...
`https://etlantic.readthedocs.io/en/v0.50.1/…` URLs for release-facing readers.
