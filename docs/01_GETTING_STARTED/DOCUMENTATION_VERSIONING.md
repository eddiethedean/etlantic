...
| Alias / slug | Meaning |
|---|---|
| **`/en/v0.52.0/`** | Immutable docs for the tagged release. Prefer this with `etlantic==0.52.0`. |
...
For `etlantic==0.52.0`, use the immutable `/en/v0.52.0/` documentation.
...
2. Activate the git tag `v0.52.0` (build if inactive).
3. Keep **latest** = `main` and **stable** = newest published tag.
4. Confirm `https://etlantic.readthedocs.io/en/v0.52.0/` returns 200.
...
`https://etlantic.readthedocs.io/en/v0.52.0/…` URLs for release-facing readers.
