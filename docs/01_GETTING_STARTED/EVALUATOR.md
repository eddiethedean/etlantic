...
| Production GUI | No — read-only-first operator console planned for 0.57 |
...
- AWS Secrets Manager / Vault (not shipped; optional provider packs planned for
  0.58); OS keyring **is** available via `etlantic-keyring`
...
- Managed Databricks/EMR/Connect Spark providers (0.47+ Experimental fakes
  shipped; live Kind/Databricks skipped; supported live packs planned for 0.58)
