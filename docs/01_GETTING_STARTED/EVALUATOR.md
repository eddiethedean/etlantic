...
| LTS | Current release-candidate minor only (`0.51.x`) |
...
## What is ready in bounded 0.51.0
...
| Production GUI | No — read-only-first operator console planned for 0.53 |
...
- Report vulnerabilities privately; security fixes are supported on 0.50.x
...
ETLantic **0.51.0** is a **Beta** (PyPI) release suitable for documented
...
- AWS Secrets Manager / Vault (not shipped; optional provider packs planned for
  0.54); OS keyring **is** available via `etlantic-keyring`
...
- Managed Databricks/EMR/Connect Spark providers (0.47+ Experimental fakes
  shipped; live Kind/Databricks skipped; supported live packs planned for 0.54)
...
| Concern | Status in 0.51 |
|---|---|
| License | MIT (core and official plugins) |
| Supported versions / EOL | Current Beta line is 0.51.x; see [SECURITY.md](https://github.com/eddiethedean/etlantic/blob/main/SECURITY.md) |
...
1. [Installation](INSTALLATION.md) — `pip install etlantic==0.51.0`
...
6. Optional Gate A: checkout
   ...
   with `etlantic-polars` + `etlantic-pandas` at `==0.51.0`
