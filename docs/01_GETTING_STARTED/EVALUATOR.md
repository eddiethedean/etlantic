...
| LTS | Current published minor only (`0.32.x`) |
...
## What is ready in bounded 0.32.0
...
- Report vulnerabilities privately; security fixes are supported on 0.32.x
...
ETLantic **0.32.0** is a **Beta** (PyPI) release suitable for documented
...
| Concern | Status in 0.32 |
|---|---|
| License | MIT (core and official plugins) |
| Supported versions / EOL | Current Beta line is 0.32.x; see [SECURITY.md](https://github.com/eddiethedean/etlantic/blob/main/SECURITY.md) |
...
| Upgrade / rollback | Pin exact versions; see [Migration 0.31 → 0.32](../11_DEVELOPMENT/MIGRATION_0_31_TO_0_32.md) and [Upgrade hub](UPGRADE.md) |
...
1. [Installation](INSTALLATION.md) — `pip install etlantic==0.32.0`
...
6. Optional Gate A: checkout
   ...
   with `etlantic-polars` + `etlantic-pandas` at `==0.32.0`
7. Optional engine examples from a checkout (portable kernels, SQL, PySpark,
   Airflow compile, Prefect)
8. [Migration 0.31 → 0.32](../11_DEVELOPMENT/MIGRATION_0_31_TO_0_32.md) if
