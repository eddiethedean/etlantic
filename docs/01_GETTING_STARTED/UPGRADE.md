...
**ETLantic 0.51.0** (Beta release candidate) — choose your guide:
...
| 0.45.x | [0.45 → 0.46](../11_DEVELOPMENT/MIGRATION_0_45_TO_0_46.md) → [0.46 → 0.47](../11_DEVELOPMENT/MIGRATION_0_46_TO_0_47.md) → [0.47 → 0.48](../11_DEVELOPMENT/MIGRATION_0_47_TO_0_48.md) |
| 0.43.x | [0.43 → 0.44](../11_DEVELOPMENT/MIGRATION_0_43_TO_0_44.md) → [0.44 → 0.45](../11_DEVELOPMENT/MIGRATION_0_44_TO_0_45.md) → [0.45 → 0.46](../11_DEVELOPMENT/MIGRATION_0_45_TO_0_46.md) |
...
| 0.47 → 0.48 | Human-governed AI context/proposals; plugin floor `etlantic>=0.48.0,<0.49` |
| 0.46 → 0.47 | Scheduler/runner service and remote federation; plugin floor `etlantic>=0.47.0,<0.48` |
| 0.45 → 0.46 | Streaming and dynamic control; plugin floor `etlantic>=0.46.0,<0.47` |
...
| From → To | Guide |
|---|---|
| 0.47 → 0.48 | [MIGRATION_0_47_TO_0_48](../11_DEVELOPMENT/MIGRATION_0_47_TO_0_48.md) |
| 0.46 → 0.47 | [MIGRATION_0_46_TO_0_47](../11_DEVELOPMENT/MIGRATION_0_46_TO_0_47.md) |
| 0.45 → 0.46 | [MIGRATION_0_45_TO_0_46](../11_DEVELOPMENT/MIGRATION_0_45_TO_0_46.md) |
...
| Do | Don't |
|---|---|
| Pin `etlantic==0.51.0` and matching plugins / `medallantic==0.51.0` | Mix 0.50 plugins with a 0.51 core |
| Keep executable Profiles explicit and treat `/2` as a wire-contract foundation | Send `/2` plans to runtimes or compilers |
| Read built-in step metrics from the `etlantic.*` keys | Assume new writers emit the 0.50 bare aliases |

See [Migration 0.50 → 0.51](../11_DEVELOPMENT/MIGRATION_0_50_TO_0_51.md).

## 0.50 configuration reference
...
| Do | Don't |
|---|---|
| Pin `etlantic==0.48.0` and matching plugins / `medallantic==0.48.0` | Mix 0.47 plugins with a 0.48 core |
| Treat proposals as untrusted until sandbox + a current 0.42 approval | Give agents schedule/DLQ/erasure/run mutation tools |
| Pin `plugin_allowlist` for `etlantic-mcp` when that extra is selected | Embed secrets, rows, or payloads in context bundles |

See [Migration 0.47 → 0.48](../11_DEVELOPMENT/MIGRATION_0_47_TO_0_48.md).

## 0.47 configuration cheat sheet

| Do | Don't |
|---|---|
| Pin `etlantic==0.47.0` and matching plugins / `medallantic==0.47.0` | Mix 0.46 plugins with a 0.47 core |
| Keep production `plugin_allowlist` and, if used, `resource_provider_allowlist` explicit | Run pipelines inside FastAPI `BackgroundTasks` or import FastAPI from workers |
| Treat Kubernetes and Spark Connect extras as Experimental fakes-first | Expect live Kind/Databricks in CI (`047-K-01` / `047-S-01`) |

See [Migration 0.46 → 0.47](../11_DEVELOPMENT/MIGRATION_0_46_TO_0_47.md).

## 0.46 configuration cheat sheet

| Do | Don't |
|---|---|
| Pin `etlantic==0.46.0` and matching plugins / `medallantic==0.46.0` | Mix 0.45 plugins with a 0.46 core |
| Keep production `plugin_allowlist` and, if used, `schema_registry_allowlist` explicit | Load Kafka/registry adapters in production with empty allowlists |
| Treat Kafka and Confluent extras as Experimental fakes-first | Put event payloads into plans, reports, or `etlantic stream` output |
| Keep `optimization_policy` at `off` unless you accept unknown-kind reject (`PMOPT112`) | Expect compilers to flatten map/branch into a static DAG |

See [Migration 0.45 → 0.46](../11_DEVELOPMENT/MIGRATION_0_45_TO_0_46.md).
