...
1. `python -m pip install 'etlantic[polars]==0.40.0'`
...
|---|---|---|---|
| Learn the model with JSON files | `etlantic==0.40.0` | `development` | [Quickstart](QUICKSTART.md) |
| JSON / CSV files | core only | file storage bindings | [File storage](../06_EXECUTION/FILE_STORAGE_TUTORIAL.md) (PyPI) |
| Fast local dataframes | `etlantic[polars]==0.40.0` | `dataframe_engine="polars"` **plus** a `"polars"` implementation | [Polars tutorial (PyPI path)](../06_EXECUTION/POLARS_TUTORIAL.md) |
| Pandas compatibility | `etlantic[pandas]==0.40.0` | `dataframe_engine="pandas"` **plus** a `"pandas"` implementation | [Pandas tutorial (PyPI path)](../06_EXECUTION/PANDAS_TUTORIAL.md) |
| Cross-engine Polars↔Pandas | `etlantic[dataframes]==0.40.0` | both plugins allowlisted | [Interchange example](../09_EXAMPLES/INTERCHANGE_POLARS_PANDAS.md) (clone) |
| Keep work in SQL | `etlantic[sql]==0.40.0` | `sql_engine="sql"` | [SQL hello (PyPI)](../06_EXECUTION/SQL_HELLO_PYPI.md) → [SQL tutorial (clone)](../06_EXECUTION/SQL_TUTORIAL.md) |
| Local Spark batch | `etlantic[pyspark]==0.40.0` | `spark_engine="pyspark"` (needs Java) | [PySpark tutorial](../06_EXECUTION/PYSPARK_TUTORIAL.md) (clone-assisted) |
| Emit Airflow DAGs | `etlantic[airflow]==0.40.0` | `orchestrator="airflow"` | [Airflow tutorial](../06_EXECUTION/AIRFLOW_TUTORIAL.md) |
| Prefect local scheduler | `etlantic[prefect]==0.40.0` | `orchestrator="prefect"` | [Prefect example](../09_EXAMPLES/PREFECT_RUN.md) (clone) |
...
2. **Pin the minor in 0.x.** Keep core and every official plugin on the same
   release (for example `etlantic==0.40.0` with `etlantic-polars==0.40.0`).
