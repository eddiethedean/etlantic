...
> **Status: Available in ETLantic 0.23.0.**
...
| Cross-engine Polars↔Pandas | `etlantic[dataframes]==0.23.0` | both plugins allowlisted | [Interchange example](../09_EXAMPLES/INTERCHANGE_POLARS_PANDAS.md) |
| Keep work in SQL | `etlantic[sql]==0.23.0` | `sql_engine="sql"` | [SQL tutorial](../06_EXECUTION/SQL_TUTORIAL.md) |
| Local Spark batch | `etlantic[pyspark]==0.23.0` | `spark_engine="pyspark"` | [PySpark tutorial](../06_EXECUTION/PYSPARK_TUTORIAL.md) |
| Emit Airflow DAGs | `etlantic[airflow]==0.23.0` | `orchestrator="airflow"` | [Airflow tutorial](../06_EXECUTION/AIRFLOW_TUTORIAL.md) |
| Prefect local scheduler | `etlantic[prefect]==0.23.0` | `orchestrator="prefect"` | [Prefect example](../09_EXAMPLES/PREFECT_RUN.md) |
...
2. **Pin the minor in 0.x.** Keep core and every official plugin on the same
   release (for example `etlantic==0.23.0` with `etlantic-polars==0.23.0`).
