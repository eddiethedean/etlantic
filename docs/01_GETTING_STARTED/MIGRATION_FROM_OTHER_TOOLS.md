...
> **Status: Available in ETLantic 0.51.0 (published Beta).** Honest scope for evaluators who
...
> [0.55](../11_DEVELOPMENT/ADOPTION_ECOSYSTEM_PLAN.md#brownfield-adoption-bridges).
...
- In 0.36, do not expect ETLantic to import a dbt project into a
  `PipelinePlan`. The planned 0.55 bridge starts from versioned
...
|---|---|
| Dagster | No compiler plugin; a Definitions compiler is planned for 0.55 |
| Prefect | Optional `etlantic-prefect` local MVP (direct execution); compile/deployment expansion is planned for 0.55 |
...
- [Capabilities](CAPABILITIES.md) — what ships in 0.51
