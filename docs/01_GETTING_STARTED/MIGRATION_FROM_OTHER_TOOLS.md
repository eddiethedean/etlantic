...
> [0.51](../11_DEVELOPMENT/ADOPTION_ECOSYSTEM_PLAN.md#brownfield-adoption-bridges).
...
- In 0.36, do not expect ETLantic to import a dbt project into a
  `PipelinePlan`. The planned 0.51 bridge starts from versioned
...
|---|---|
| Dagster | No compiler plugin; a Definitions compiler is planned for 0.51 |
| Prefect | Optional `etlantic-prefect` local MVP (direct execution); compile/deployment expansion is planned for 0.51 |
...
- [Capabilities](CAPABILITIES.md) — what ships in 0.48
