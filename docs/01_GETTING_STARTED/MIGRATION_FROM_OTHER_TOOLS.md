...
> **Status: ETLantic 0.51.0 release candidate; publication pending.** Honest scope for evaluators who
...
- In 0.36, do not expect ETLantic to import a dbt project into a
  `PipelinePlan`. The planned 0.52 bridge starts from versioned
...
|---|---|
| Dagster | No compiler plugin; a Definitions compiler is planned for 0.52 |
| Prefect | Optional `etlantic-prefect` local MVP (direct execution); compile/deployment expansion is planned for 0.52 |
