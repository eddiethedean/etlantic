...
ETLantic 0.28.0 discovers portable transformation compilers through the
...
dependencies = ["etlantic>=0.28.0,<0.29", "acme-frame>=2,<3"]
...
Install the wheel into an isolated environment with ETLantic 0.28.0, then:
...
Depend on a matching ETLantic minor (`etlantic>=0.28.0,<0.29` for official
0.20 plugins).
