...
ETLantic 0.23.0 discovers portable transformation compilers through the
...
dependencies = ["etlantic>=0.23.0,<0.24", "acme-frame>=2,<3"]
...
Install the wheel into an isolated environment with ETLantic 0.23.0, then:
...
Depend on a matching ETLantic minor (`etlantic>=0.23.0,<0.24` for official
0.20 plugins).
