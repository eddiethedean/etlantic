...
> **Status (0.39.0):** Plugin `/1` families remain **frozen since 0.28.0** on
...
> stable-foundation claim. External feedback evidence:
...
| Surface | Decision | Evidence / notes |
|---|---|---|
...
See [FINDINGS_0_36](../11_DEVELOPMENT/FINDINGS_0_36.md),
[EXIT_GATE_0_36](../11_DEVELOPMENT/EXIT_GATE_0_36.md), and
...
| No unresolved provisional core protocol on the 0.38 path | Met in 0.28 with `scheduler/1` deferred; **0.37** promotes `scheduler/1` to stable MVP and keeps `quality/1` provisional | Maintainers |
...
- **Core minor pin:** plugin packages should declare
  `etlantic>=X.Y,<X.(Y+1)` (for 0.38: `etlantic>=0.39.0,<0.40`).
