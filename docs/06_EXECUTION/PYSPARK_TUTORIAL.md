...
> **Status: Available in ETLantic 0.53.0 (Beta release candidate).** Structured Streaming remains
> experimental.
...
```bash
python -m pip install 'etlantic==0.53.0' 'etlantic-pyspark==0.53.0'
git clone --branch v0.53.0 https://github.com/eddiethedean/etlantic.git
cd etlantic
...
```bash
python -m pip install 'etlantic-pyspark[delta]==0.53.0'
