...
> **Status: ETLantic 0.51.0 release candidate; publication pending.** Embed `etlantic-fastapi` in a host
...
```bash
python -m pip install 'etlantic-fastapi==0.51.0'
# or: python -m pip install 'etlantic[fastapi]==0.51.0'
```

Pin the same minor as core (`etlantic==0.51.0`). Package README:
