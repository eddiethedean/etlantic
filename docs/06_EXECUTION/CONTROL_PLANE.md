...
> **Status: Available in ETLantic 0.54.0 (Beta release candidate).** Embed `etlantic-fastapi` in a host
...
```bash
python -m pip install 'etlantic-fastapi==0.54.0'
# or: python -m pip install 'etlantic[fastapi]==0.54.0'
```

Pin the same minor as core (`etlantic==0.54.0`). Package README:
