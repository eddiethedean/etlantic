...
> **Status: Available in ETLantic 0.46.0 (incubation).** Embeddable HTTP control
...
```bash
python -m pip install 'etlantic-fastapi==0.46.0'
# or: python -m pip install 'etlantic[fastapi]==0.46.0'
```

Pin the same minor as core (`etlantic==0.46.0`). Package README:
