...
> **Status: Available in ETLantic 0.54.0 (Beta release candidate).** ETLantic compiles a plan; it does
...
!!! note "Repository examples"
    ...
    wheel. Clone a matching checkout (prefer the `v0.54.0` tag) and use
    `uv sync` / the documented dependency group before running them.

## Install and compile

```bash
python -m pip install 'etlantic==0.54.0' 'etlantic-airflow==0.54.0'
git clone --branch v0.54.0 https://github.com/eddiethedean/etlantic.git
cd etlantic
