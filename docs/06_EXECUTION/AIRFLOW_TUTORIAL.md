...
> **Status: Available in ETLantic 0.35.0.** ETLantic compiles a plan; it does
...
!!! note "Repository examples"
    ...
    wheel. Clone a matching checkout (prefer the `v0.35.0` tag) and use
    `uv sync` / the documented dependency group before running them.

## Install and compile

```bash
python -m pip install 'etlantic==0.35.0' 'etlantic-airflow==0.35.0'
