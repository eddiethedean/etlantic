...
| 2 | Ordinary explicit profiles retain canonical `/1` bytes, fingerprints, semantics, and reader behavior; opted-in fallback adds only `etlantic.adaptive_fallback` metadata | Pass | **Foundation verified for explicit Profiles; fallback is not implemented and remains pending** | #42–#44 |
| 3 | Unsupported old readers and all unqualified `/2` consumers reject before external I/O | Pass | **Foundation rejection tests pass; full consumer artifact pending** | #44, #90, #92, #94 |
...
| 9 | All seven physical-unit kinds validate, round-trip, preserve their frozen effect/ownership/retry semantics, reject tampering, and remain secret/source-row free | Pass | **Wire-model validation and round-trip tests pass; executable semantics remain pending** | #43, #64–#68, #93 |
...
| 17 | Plans, evidence, diagnostics, explain artifacts, and reports contain no resolved secrets or source rows | Pass | **Foundation `/2` and Profile wire sections reject secret/source-row material; later artifacts remain pending** | #44, #68, #77, #82 |
| 18 | Concepts, operations, rollback, plugin, migration, reference, release, and executable-example documentation passes strict checks | Pass | **Foundation release documentation passes; full adaptive operations/examples remain pending** | #83–#87, #94 |
| 19 | Runtime writers emit only `etlantic.dataframe`, `etlantic.sql`, `etlantic.spark`, and `etlantic.spark_schema`; 0.50 bare aliases load warning-free, namespaced values win collisions, bare aliases are removed, and migration is deterministic and idempotent | Pass | **Implemented and covered by compatibility tests** | #41, #69–#73, #83–#87, #94 |
| 20 | No unresolved critical/high security, correctness, compatibility, or data-loss finding | 0 | **Foundation increment independently reviewed with no remaining blocker** | #82, #95 |
...
|---|---|---|
| `adaptive_wire_compatibility_0_51.json` | `/1` byte/fingerprint goldens; `/1`-only `PipelinePlan`; `/2` `AdaptivePipelinePlan`; dispatching reader/writer/verify/JSON-Schema matrix; fallback metadata isolation | Executable foundation tests pass; consolidated full-gate artifact pending |
| `runtime_metadata_namespace_compatibility_0_51.json` | Namespaced-only writer output; 0.50 fixtures for `dataframe`, `sql`, `spark`, and `spark_schema`; namespaced-wins collisions; warning-free deterministic/idempotent migration and reserialization | Executable compatibility tests pass; consolidated full-gate artifact pending |
...
| `MIGRATION_0_50_TO_0_51.md` | Opt-in profile migration, `/1`–`/2`, runtime-report metadata aliases and collision behavior, rollback, and consumer compatibility | Prepared for foundation release |
| `WHATS_NEW_0_51.md` | Exact Available matrix and explicit non-claims | Prepared for foundation release |
