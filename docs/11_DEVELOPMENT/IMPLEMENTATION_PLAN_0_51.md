...
raise them in 0.51. The limit-set version and effective values participate in
the `/2` fingerprint.
...
| DuckDB only | None | Local DuckDB target | Experimental candidate after the 0.50 seven-engine/pushdown gate and single-target physical-DAG evidence; no default availability claim |
...
0.51 adaptive availability claim. DuckDB is also Experimental unless its
single-target row passes the 0.50 seven-engine/pushdown gate and this phase's
physical-DAG evidence. All unqualified combinations fail closed unless a later
gate adds a qualified `/2` physical-DAG consumer and combination row.
