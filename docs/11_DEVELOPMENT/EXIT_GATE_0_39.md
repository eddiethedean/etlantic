...
| `etlantic-fastapi` embeddable router / app factory + OpenAPI 3.1 | **Met** |
...
| Durable `202` acceptance + scoped idempotency | **Met** (`test_cp1_durable.py`) |
...
| Optional SQLModel reference stores (request-scoped) | **Met** (`etlantic_sqlmodel.control_plane`) |
...
|---|---:|---|
| Existing FastAPI app embeds router without replacing lifespan, DI, middleware, or exception handling | Pass | **Met** (`include_router`; durable embed test) |
| OpenAPI 3.1 stable with deterministic operation IDs | Pass | **Met** (`openapi_cp1_snapshot.json`) |
| Generated client completes reference workflow | Pass | **Met** (OpenAPI dump + happy-path smoke) |
| Two API workers share durable submissions and event history | Pass | **Met** (shared store + threadpool idempotency) |
...
| OpenAPI snapshot | `tests/fastapi/openapi_cp1_snapshot.json` |
| Authz matrix | `tests/fastapi/test_cp1_full_authz_matrix.py` |
| Redaction / SSE / landing / durable | `tests/fastapi/test_cp1_{redaction,sse,landing,durable}.py` |
...
- [x] `202 Accepted` + accept receipt from durable store
...
- [x] Optional SQLModel stores request-scoped
