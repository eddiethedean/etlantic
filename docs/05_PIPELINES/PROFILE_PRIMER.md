...
> **Status: Available in ETLantic 0.53.0 (Beta release candidate).** Bind a portable pipeline to an environment.
...
profile = production_profile(
    plugin_allowlist={"etlantic-polars": "==0.53.0", "local": None},
...
`etlantic-polars==0.53.0` with `etlantic==0.53.0`).
