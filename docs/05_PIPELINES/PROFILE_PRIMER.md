...
> **Status: Available in ETLantic 0.52.0 (published Beta).** Bind a portable pipeline to an environment.
...
profile = production_profile(
    plugin_allowlist={"etlantic-polars": "==0.52.0", "local": None},
...
`etlantic-polars==0.52.0` with `etlantic==0.52.0`).
