...
> **Status: Available in ETLantic 0.54.0 (Beta release candidate).** Bind a portable pipeline to an environment.
...
profile = production_profile(
    plugin_allowlist={"etlantic-polars": "==0.54.0", "local": None},
...
`etlantic-polars==0.54.0` with `etlantic==0.54.0`).
