...
> **Status: ETLantic 0.51.0 release candidate; publication pending.** Bind a portable pipeline to an environment.
...
profile = production_profile(
    plugin_allowlist={"etlantic-polars": "==0.51.0", "local": None},
...
`etlantic-polars==0.51.0` with `etlantic==0.51.0`).
