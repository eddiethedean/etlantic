...
> **Status: The ETLantic 0.50 technical qualification for Local, Polars, Pandas, SQL, PySpark, DataFusion, and DuckDB is retained in 0.52.**
...
| `etlantic-polars==0.52.0` | Polars | kernel `/1`, relational `/1`, `portable-string-advanced/1`, `portable-conversion/1`, `portable-statistics/1`, `portable-window/1`, `portable-complex-values/1`, `portable-complex-types/1`, `portable-reshape/1` | Eager and lazy | `fail` only |
| `etlantic-pyspark==0.52.0` | PySpark | kernel `/1`, relational `/1`, `portable-string-advanced/1`, `portable-conversion/1`, `portable-statistics/1`, `portable-window/1`, `portable-complex-values/1`, `portable-complex-types/1`, `portable-reshape/1` | Eager and lazy | `fail` only |
| `etlantic-pandas==0.52.0` | Pandas | kernel `/1`, relational `/1` | Eager only (`lazy=False`) | `fail` only |
| `etlantic-sql==0.52.0` | SQL | kernel `/1`, relational `/1` | Eager (relation/SQL) | `fail` only |
| `etlantic-datafusion==0.52.0` | DataFusion | kernel `/1`, relational `/1` | Eager and lazy | `fail` only |
| `etlantic-duckdb==0.52.0` | DuckDB | kernel `/1`, relational `/1` | Lazy relation handles | `fail` only |
