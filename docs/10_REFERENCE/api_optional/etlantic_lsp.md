...
> **Status: Available in ETLantic 0.53.0 (Beta release candidate).** Editor-neutral language server
> wrapping `etlantic.ide` analysis. Install via `etlantic[lsp]` or
> `etlantic-lsp==0.53.0`. VS Code reference client remains **Experimental**.

## Setup

```bash
pip install 'etlantic[lsp]==0.53.0'
etlantic-lsp
