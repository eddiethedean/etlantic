...
ETLantic **0.39.x** is a **Beta** (PyPI) release suitable for documented
single-tenant reference deployments. 0.x releases may still introduce breaking
API changes between minor versions. **CP1** (`etlantic.control_plane` /
`etlantic-fastapi` `ETLanticAPI`) is identity/API incubation — **not**
production multi-tenant GA (graduation remains **0.43**).

| ID | Component | Affected | Symptom | Workaround | Status |
|---|---|---|---|---|---|
| DOC-001 | Release maturity | All adopters | Breaking API changes possible between 0.x minors | Pin `etlantic==0.39.0` and matching plugins; follow Upgrade hub | Open (Beta) |
| DOC-026 | Control plane (CP1) | Multi-tenant hosts | Treating CP1 as GA isolation / durable execution | Use CP1 for embeddable identity + durable accept only; graduation evidence is 0.43 | By design |
