...
ETLantic **0.40.x** is a **Beta** (PyPI) release suitable for documented
single-tenant reference deployments. 0.x releases may still introduce breaking
API changes between minor versions. **CP2** (`etlantic.control_plane` registry
+ optional `etlantic-fastapi` / `etlantic-sqlmodel` / `etlantic-openlineage`)
is registry/persistence incubation — **not** production multi-tenant GA
(graduation remains **0.43**).

| ID | Component | Affected | Symptom | Workaround | Status |
|---|---|---|---|---|---|
| DOC-001 | Release maturity | All adopters | Breaking API changes possible between 0.x minors | Pin `etlantic==0.40.0` and matching plugins; follow Upgrade hub | Open (Beta) |
