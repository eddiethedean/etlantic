...
This table describes the declared compatibility of ETLantic 0.53.0.
...
| FastAPI reference adapter | `etlantic-fastapi==0.53.0` (reference only; not a control plane; see the [planned first-class control-plane program](../11_DEVELOPMENT/MULTI_TENANT_CONTROL_PLANE_PLAN.md)) |
...
`etlantic==0.53.0` plus every official plugin to `==0.53.0`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.53.0` depends on `etlantic-polars==0.53.0`. Official
plugin source metadata accepts core `etlantic>=0.53.0,<0.54`, which is
