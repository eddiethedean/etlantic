...
This table describes the declared compatibility of ETLantic 0.24.0.
...
| Polars plugin | `etlantic-polars==0.24.0` |
| Pandas plugin | `etlantic-pandas==0.24.0` |
| SQL plugin | `etlantic-sql==0.24.0` |
| PySpark plugin | `etlantic-pyspark==0.24.0` |
| Airflow plugin | `etlantic-airflow==0.24.0` |
| Prefect scheduler | `etlantic-prefect==0.24.0` |
| Keyring provider | `etlantic-keyring==0.24.0` |
| SQLModel bridge | `etlantic-sqlmodel==0.24.0` |
| SparkForge adapter | `etlantic-sparkforge==0.24.0` |
| DataFusion plugin | `etlantic-datafusion==0.24.0` (**Experimental** Gate B stub; no graduated claims) |
...
| Plugin SDK stability | Shipped protocols stable within the 0.23 minor; third-party plugins must pin and re-run conformance |
...
`etlantic==0.24.0` plus every official plugin to `==0.24.0`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.24.0` depends on `etlantic-polars==0.24.0`. Official
plugin source metadata accepts core `etlantic>=0.24.0,<0.25`, which is
