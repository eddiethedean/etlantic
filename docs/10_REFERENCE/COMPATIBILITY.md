...
This table describes the declared compatibility of ETLantic 0.52.1.
...
| Polars plugin | `etlantic-polars==0.52.1` |
| Pandas plugin | `etlantic-pandas==0.52.1` |
| SQL plugin | `etlantic-sql==0.52.1` |
| DuckDB plugin | `etlantic-duckdb==0.52.1` (optional) |
| PySpark plugin | `etlantic-pyspark==0.52.1` |
| Airflow plugin | `etlantic-airflow==0.52.1` |
| Prefect scheduler | `etlantic-prefect==0.52.1` |
| Keyring provider | `etlantic-keyring==0.52.1` |
| SQLModel bridge | `etlantic-sqlmodel==0.52.1` |
| FastAPI reference adapter | `etlantic-fastapi==0.52.1` (reference only; not a control plane; see the [planned first-class control-plane program](../11_DEVELOPMENT/MULTI_TENANT_CONTROL_PLANE_PLAN.md)) |
| Medallantic (facade) | `medallantic==0.52.1` |
| SparkForge redirect | `etlantic-sparkforge==0.52.1` (compatibility shim; prefer `medallantic`) |
| DataFusion plugin | `etlantic-datafusion==0.52.1` (technical qualification complete; provisional Alpha surface) |
...
| Plugin SDK stability | Protocol `/1` families **frozen** in 0.28.0; third-party plugins must pin `etlantic>=0.52.1,<0.53` and re-run conformance |
...
| Core + official plugins + Medallantic | Joint pins on the same minor (`==0.52.1`) |
...
`etlantic==0.52.1` plus every official plugin to `==0.52.1`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.52.1` depends on `etlantic-polars==0.52.1`. Official
plugin source metadata accepts core `etlantic>=0.52.1,<0.53`, which is
