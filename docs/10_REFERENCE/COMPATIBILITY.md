...
This table describes the declared compatibility of ETLantic 0.52.0.
...
| Polars plugin | `etlantic-polars==0.52.0` |
| Pandas plugin | `etlantic-pandas==0.52.0` |
| SQL plugin | `etlantic-sql==0.52.0` |
| DuckDB plugin | `etlantic-duckdb==0.52.0` (optional) |
| PySpark plugin | `etlantic-pyspark==0.52.0` |
| Airflow plugin | `etlantic-airflow==0.52.0` |
| Prefect scheduler | `etlantic-prefect==0.52.0` |
| Keyring provider | `etlantic-keyring==0.52.0` |
| SQLModel bridge | `etlantic-sqlmodel==0.52.0` |
| FastAPI reference adapter | `etlantic-fastapi==0.52.0` (reference only; not a control plane; see the [planned first-class control-plane program](../11_DEVELOPMENT/MULTI_TENANT_CONTROL_PLANE_PLAN.md)) |
| Medallantic (facade) | `medallantic==0.52.0` |
| SparkForge redirect | `etlantic-sparkforge==0.52.0` (compatibility shim; prefer `medallantic`) |
| DataFusion plugin | `etlantic-datafusion==0.52.0` (technical qualification complete; provisional Alpha surface) |
...
| Plugin SDK stability | Protocol `/1` families **frozen** in 0.28.0; third-party plugins must pin `etlantic>=0.52.0,<0.53` and re-run conformance |
...
| Core + official plugins + Medallantic | Joint pins on the same minor (`==0.52.0`) |
...
`etlantic==0.52.0` plus every official plugin to `==0.52.0`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.52.0` depends on `etlantic-polars==0.52.0`. Official
plugin source metadata accepts core `etlantic>=0.52.0,<0.53`, which is
