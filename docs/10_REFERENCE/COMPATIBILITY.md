...
This table describes the declared compatibility of ETLantic 0.23.0.
...
| Polars plugin | `etlantic-polars==0.23.0` |
| Pandas plugin | `etlantic-pandas==0.23.0` |
| SQL plugin | `etlantic-sql==0.23.0` |
| PySpark plugin | `etlantic-pyspark==0.23.0` |
| Airflow plugin | `etlantic-airflow==0.23.0` |
| Prefect scheduler | `etlantic-prefect==0.23.0` |
...
`etlantic==0.23.0` plus every official plugin to `==0.23.0`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.23.0` depends on `etlantic-polars==0.23.0`. Official
plugin source metadata accepts core `etlantic>=0.23.0,<0.24`, which is
