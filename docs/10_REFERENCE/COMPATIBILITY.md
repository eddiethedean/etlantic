...
This table describes the declared compatibility of ETLantic 0.37.0.
...
[Capabilities — Supported standards policy](../01_GETTING_STARTED/CAPABILITIES.md#supported-standards-policy-037).
...
| Polars plugin | `etlantic-polars==0.37.0` |
| Pandas plugin | `etlantic-pandas==0.37.0` |
| SQL plugin | `etlantic-sql==0.37.0` |
| PySpark plugin | `etlantic-pyspark==0.37.0` |
| Airflow plugin | `etlantic-airflow==0.37.0` |
| Prefect scheduler | `etlantic-prefect==0.37.0` |
| Keyring provider | `etlantic-keyring==0.37.0` |
| SQLModel bridge | `etlantic-sqlmodel==0.37.0` |
| FastAPI reference adapter | `etlantic-fastapi==0.37.0` (reference only; not a control plane; see the [planned first-class control-plane program](../11_DEVELOPMENT/MULTI_TENANT_CONTROL_PLANE_PLAN.md)) |
| Medallantic (facade) | `medallantic==0.37.0` |
| SparkForge redirect | `etlantic-sparkforge==0.37.0` (compatibility shim; prefer `medallantic`) |
| DataFusion plugin | `etlantic-datafusion==0.37.0` (**Experimental** Gate B stub; no graduated claims) |
...
| Plugin SDK stability | Protocol `/1` families **frozen** in 0.28.0; third-party plugins must pin `etlantic>=0.37.0,<0.38` and re-run conformance |
...
| Core + official plugins + Medallantic | Joint pins on the same minor (`==0.37.0`) |
...
`etlantic==0.37.0` plus every official plugin to `==0.37.0`.

Core extras already enforce exact official plugin versions, for example
`etlantic[polars]==0.37.0` depends on `etlantic-polars==0.37.0`. Official
plugin source metadata accepts core `etlantic>=0.37.0,<0.38`, which is
