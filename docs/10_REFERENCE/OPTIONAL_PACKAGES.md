...
> **Status: Available in ETLantic 0.44.0.** Core `etlantic` does not install
...
pip install 'etlantic-pandas==0.44.0'
pip install 'etlantic-sql==0.44.0'
pip install 'etlantic-pyspark==0.44.0'
pip install 'etlantic-airflow==0.44.0'
pip install 'etlantic-prefect==0.44.0'
pip install 'etlantic-keyring==0.44.0'
pip install 'etlantic-sqlmodel==0.44.0'
pip install 'medallantic==0.44.0'
# optional compatibility redirect (deprecated):
pip install 'etlantic-sparkforge==0.44.0'
...
Official first-party plugins declare `etlantic>=0.44.0,<0.45`.
Keep core and plugins on the same minor (pin all to `0.44.0` for pilots).
Cross-minor mixes are unsupported and commonly fail plugin discovery.

Experimental (not recommended): `pip install 'etlantic[datafusion]==0.44.0'`
or `etlantic-datafusion==0.44.0` — Gate B stub; no graduated claims.

Optional FastAPI package: `pip install 'etlantic-fastapi==0.44.0'` or
`etlantic[fastapi]==0.44.0` — **dual surface**: CP1 (`ETLanticAPI` /
...
in 0.44 (`isolated-deployment`, `dedicated-schema`; `shared-service`
