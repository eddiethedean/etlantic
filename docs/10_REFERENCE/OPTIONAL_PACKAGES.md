...
> **Status: Available in ETLantic 0.45.0.** Core `etlantic` does not install
...
!!! warning "Maturity vs PyPI classifiers"
    ...
    **ETLantic 0.45 Beta** envelope (community non-SLA). Treat that envelope as
...
pip install 'etlantic-polars==0.45.0'
pip install 'etlantic-pandas==0.45.0'
pip install 'etlantic-sql==0.45.0'
pip install 'etlantic-pyspark==0.45.0'
pip install 'etlantic-airflow==0.45.0'
pip install 'etlantic-prefect==0.45.0'
pip install 'etlantic-keyring==0.45.0'
pip install 'etlantic-sqlmodel==0.45.0'
pip install 'medallantic==0.45.0'
# optional compatibility redirect (deprecated):
pip install 'etlantic-sparkforge==0.45.0'
# experimental connectors (fake/CI; Alpha — not Supported):
pip install 'etlantic-s3==0.45.0'
pip install 'etlantic-iceberg==0.45.0'
pip install 'etlantic-snowflake==0.45.0'
```

Official first-party plugins declare `etlantic>=0.45.0,<0.46`.
Keep core and plugins on the same minor (pin all to `0.45.0` for pilots).
Cross-minor mixes are unsupported and commonly fail plugin discovery.

Experimental (not recommended): `pip install 'etlantic[datafusion]==0.45.0'`
or `etlantic-datafusion==0.45.0` — Gate B stub; no graduated claims.

Optional FastAPI package: `pip install 'etlantic-fastapi==0.45.0'` or
`etlantic[fastapi]==0.45.0` — **dual surface**: CP1 (`ETLanticAPI` /
...
| [`etlantic-fastapi`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-fastapi/README.md) | `etlantic_fastapi` | Dual surface: CP1 `ETLanticAPI` + thin `create_reference_app` (pin `==0.45.0`; CPn alone ≠ GA) |
| [`etlantic-lsp`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-lsp/README.md) | `etlantic_lsp` | Editor-neutral language server (`etlantic[lsp]` / pin `==0.45.0`) |
