...
> **Status: Available in ETLantic 0.35.0.** Core `etlantic` does not install
...
pip install 'etlantic-polars==0.35.0'
pip install 'etlantic-pandas==0.35.0'
pip install 'etlantic-sql==0.35.0'
pip install 'etlantic-pyspark==0.35.0'
pip install 'etlantic-airflow==0.35.0'
pip install 'etlantic-prefect==0.35.0'
pip install 'etlantic-keyring==0.35.0'
pip install 'etlantic-sqlmodel==0.35.0'
pip install 'medallantic==0.35.0'
# optional compatibility redirect (deprecated):
pip install 'etlantic-sparkforge==0.35.0'
```

Official first-party plugins declare `etlantic>=0.35.0,<0.36`.
Keep core and plugins on the same minor (pin all to `0.35.0` for pilots).
Cross-minor mixes are unsupported and commonly fail plugin discovery.

Experimental (not recommended): `pip install 'etlantic[datafusion]==0.35.0'`
or `etlantic-datafusion==0.35.0` — Gate B stub; no graduated claims.

Optional reference adapter: `pip install 'etlantic-fastapi==0.35.0'` or
`etlantic[fastapi]==0.35.0` — thin authoring/service HTTP adapter (shipped since
0.24; not the
...
| [`etlantic-fastapi`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-fastapi/README.md) | `etlantic_fastapi` | Thin FastAPI authoring/service reference adapter (shipped since 0.24; pin `==0.35.0`) |
