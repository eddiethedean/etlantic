...
> **Status: Available in ETLantic 0.52.0 (published Beta).** Core `etlantic` does not install
...
!!! warning "Maturity vs PyPI classifiers"
    ...
    **ETLantic 0.49 Beta** envelope (community non-SLA). Treat that envelope as
...
pip install 'etlantic-polars==0.52.0'
pip install 'etlantic-pandas==0.52.0'
pip install 'etlantic-sql==0.52.0'
pip install 'etlantic-pyspark==0.52.0'
pip install 'etlantic-airflow==0.52.0'
pip install 'etlantic-prefect==0.52.0'
pip install 'etlantic-keyring==0.52.0'
pip install 'etlantic-sqlmodel==0.52.0'
pip install 'medallantic==0.52.0'
# optional compatibility redirect (deprecated):
pip install 'etlantic-sparkforge==0.52.0'
# experimental connectors (fake/CI; Alpha — not Supported):
pip install 'etlantic-s3==0.52.0'
pip install 'etlantic-iceberg==0.52.0'
pip install 'etlantic-snowflake==0.52.0'
pip install 'etlantic-k8s==0.52.0'
pip install 'etlantic-spark-connect==0.52.0'
pip install 'etlantic-mcp==0.52.0'
```

Official first-party plugins declare `etlantic>=0.52.0,<0.53`.
Keep core and plugins on the same patch (pin all to `0.52.0` for pilots).
Cross-minor mixes are unsupported and commonly fail plugin discovery.

DataFusion: `pip install 'etlantic[datafusion]==0.52.0'` or
`etlantic-datafusion==0.52.0` — provisional portable dataframe/compiler implementation (0.50 technical qualification recorded; independent review pending).

Optional FastAPI package: `pip install 'etlantic-fastapi==0.52.0'` or
`etlantic[fastapi]==0.52.0` — **dual surface**: CP1 (`ETLanticAPI` /
...
| [`etlantic-mcp`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-mcp/README.md) | `etlantic_mcp` | **Experimental** read-only MCP extra (FakeMcpServer; Alpha) |
...
| [`etlantic-fastapi`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-fastapi/README.md) | `etlantic_fastapi` | Dual surface: CP1 `ETLanticAPI` + thin `create_reference_app` (pin `==0.52.0`; CPn alone ≠ GA) |
| [`etlantic-lsp`](https://github.com/eddiethedean/etlantic/blob/main/packages/etlantic-lsp/README.md) | `etlantic_lsp` | Editor-neutral language server (`etlantic[lsp]` / pin `==0.52.0`) |
