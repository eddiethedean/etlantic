...
!!! success "**Status: Available in ETLantic 0.53.0 (Beta release candidate)**"
...
This page is **PyPI-pasteable**: install packages, save the script below, run it.
Repository path `examples/interchange_polars_pandas.py` is for contributors with
a checkout (and CI); it is **not** included in the wheel.

## 1. Install

```bash
python -m pip install 'etlantic[dataframes]==0.53.0'
```

## 2. Create `interchange_polars_pandas.py`

Copy the complete script below (or download the tagged raw file).
...
Or from published packages:
    ...
    pip install 'etlantic[dataframes]==0.53.0'
...
def _require_plugins() -> dict[str, Any]:
    ...
    if missing:
        raise SystemExit(
            ...
            "or pip install 'etlantic[dataframes]==0.53.0'"
        )
    return found
...
curl -fsSL -o interchange_polars_pandas.py \
  https://raw.githubusercontent.com/eddiethedean/etlantic/v0.53.0/examples/interchange_polars_pandas.py
python interchange_polars_pandas.py
```
...
```text
Gate A interchange demo: SUCCESS
  schema=etlantic.interchange/1
```

Additional lines print mechanism, engines, rows, and the run report. The run must
