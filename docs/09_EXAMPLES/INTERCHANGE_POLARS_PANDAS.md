...
!!! success "**Status: Available in ETLantic 0.45.0**"
...
```bash
python -m pip install 'etlantic[dataframes]==0.45.0'
```
...
Or from published packages:
    ...
    pip install 'etlantic[dataframes]==0.45.0'
...
def _require_plugins() -> dict[str, Any]:
    ...
    if missing:
        raise SystemExit(
            ...
            "or pip install 'etlantic[dataframes]==0.45.0'"
        )
    return found
...
curl -fsSL -o interchange_polars_pandas.py \
  https://raw.githubusercontent.com/eddiethedean/etlantic/v0.45.0/examples/interchange_polars_pandas.py
python interchange_polars_pandas.py
```
