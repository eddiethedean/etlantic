...
> **Status: Available in ETLantic 0.52.0 (published Beta).** This guide runs the shipped Prefect
...
!!! warning "Clone required"
    ...
    (prefer the `v0.52.0` tag) plus `uv sync --group prefect`.
...
```bash
pip install 'etlantic==0.52.0' 'etlantic-prefect==0.52.0'
```
