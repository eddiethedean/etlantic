...
> **Status: ETLantic 0.51.0 release candidate; publication pending.** This guide runs the shipped Prefect
...
!!! warning "Clone required"
    ...
    (prefer the `v0.51.0` tag) plus `uv sync --group prefect`.
...
```bash
pip install 'etlantic==0.51.0' 'etlantic-prefect==0.51.0'
```
