...
> **Status: Available in ETLantic 0.49.0.** This guide runs the shipped Prefect
...
!!! warning "Clone required"
    ...
    (prefer the `v0.49.0` tag) plus `uv sync --group prefect`.
...
```bash
pip install 'etlantic==0.49.0' 'etlantic-prefect==0.49.0'
```
