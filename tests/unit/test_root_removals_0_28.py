#!/usr/bin/env python3
"""Tests for 0.28 root alias removals and import hygiene."""
...
import etlantic
from etlantic import _REMOVED_0_28

ROOT = Path(__file__).resolve().parents[2]
SCAN_ROOTS = (ROOT / "tests", ROOT / "examples")


@pytest.mark.parametrize("name", sorted(_REMOVED_0_28))
def test_removed_root_alias_raises(name: str) -> None:
    with pytest.raises(
        AttributeError, match=r"removed from the etlantic root in 0\.28\.0"
    ):
...
def test_tests_and_examples_do_not_import_removed_root_symbols() -> None:
    ...
    assert not violations, (
        "tests/examples still import 0.28-removed root symbols from etlantic:\n"
        + "\n".join(violations)
    )
