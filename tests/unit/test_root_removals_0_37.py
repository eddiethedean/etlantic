#!/usr/bin/env python3
"""Tests for 0.37 root alias removals and import hygiene."""
...
import etlantic
from etlantic import _REMOVED_0_37

ROOT = Path(__file__).resolve().parents[2]
SCAN_ROOTS = (ROOT / "tests", ROOT / "examples")


@pytest.mark.parametrize("name", sorted(_REMOVED_0_37))
...
def test_tests_and_examples_do_not_import_removed_root_symbols() -> None:
    ...
    assert not violations, (
        "tests/examples still import 0.37-removed root symbols from etlantic:\n"
        + "\n".join(violations)
    )
