...
| | |
|---|---|
| Who must act | Users generating new projects with `etlantic init`, following default examples, or planning portable transformations loaded from `etlantic.pipeline/1` documents |
| Breaking | None; wire schemas, qualified compiler identities, and the 0.50 plugin compatibility floor are unchanged, and the runtime metadata namespace rename remains deferred to 0.51 |
| Upgrade | Install `etlantic==0.50.1` with matching first-party plugins; existing 0.50 plans, compiler evidence, and reports remain valid |
| Rollback | Re-pin the complete 0.50.0 lockstep set; projects generated by 0.50.1 remain valid portable pipelines on the 0.50 line |
| Security | No security-boundary change; plans, reports, and examples remain subject to the existing secret and source-row exclusions |

### Changed

- `etlantic init`, the getting-started guides, and general runnable examples
  now recommend `@Transformation.portable` with
  `portable_transform_policy="require"`; native implementations are documented
  as engine-specific escape hatches that are ineligible for adaptive execution.

### Fixed

- Planning a portable transformation loaded from an
  `etlantic.pipeline/1` document now makes a fully mutable plan copy before
  fingerprinting, avoiding nested `mappingproxy` failures.
- Built-in dataframe, SQL, and Spark step metrics reload without spurious
